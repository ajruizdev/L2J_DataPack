...
public class Q00144_PailakaInjuredDragon extends Quest {
	...
	private static final int ZONE = 45;
	...
	// Arrays
	private static final int[] NPCS = { KETRA_ORC_SHAMAN, KETRA_ORC_SUPPORTER,
			KETRA_ORC_INTELIGENCE_OFFICER, KETRA_ORC_SUPPORTER2 };
	private static final int[] MONSTERS = { 18635, 18636, 18638, 18639, 18640,
			18641, 18642, 18644, 18645, 18646, 18648,
			VARKA_SILENOS_GREAT_MAGUS, 18650, 18652, 18653, VARKAS_COMMANDER,
			18655, 18656, 18657, 18658, 18659, ANTYLOPE_1, ANTYLOPE_2,
			ANTYLOPE_3, ANTYLOPE_4 };
	private static final int[] ITEMS = { SPEAR, ENCHSPEAR, LASTSPEAR, STAGE1,
			STAGE2, SHIELD_POTION, HEAL_POTION };
	...
	private static final int[][] DROPLIST = {
			...
			// npcId, itemId, chance
			{ HEAL_POTION, 80 }, { SHIELD_POTION, 30 } };
	...
	public Q00144_PailakaInjuredDragon() {
		super(144, Q00144_PailakaInjuredDragon.class.getSimpleName(),
				"Pailaka - Inujured Dragon");
	...
	private static final void dropHerb(L2Npc mob, L2PcInstance player,
		...
		for (int[] element : drop) {
			if (chance < element[2]) {
				((L2MonsterInstance) mob).dropItem(player, element[0],
						element[1]);
				return;
			}
		}
	...
	private static final void dropItem(L2Npc mob, L2PcInstance player) {
		...
		for (int[] drop : DROPLIST) {
			if (chance < drop[1]) {
				((L2MonsterInstance) mob).dropItem(player, drop[0],
						getRandom(1, 6));
				return;
			}
		}
	...
	private static void giveBuff(L2Npc npc, L2PcInstance player, int skillId,
			int level) {
	...
	public final String onAdvEvent(String event, L2Npc npc, L2PcInstance player) {
		...
		if (event.equalsIgnoreCase("enter")) {
			...
			return null;
		} else if (event.equalsIgnoreCase("32499-02.htm")) // Shouldn't be
															// 32499-04.htm
		// ???
		{
			if (cond == 0) {
			...
			case 6:
				...
				addSpawn(LATANA, 105732, -41787, -1782, 35742, false, 0, false,
						npc.getInstanceId());
				// st.playSound("ItemSound.quest_itemget");
				return "32509-03.htm";
			case 7:
	...
	@Override
	public final String onAttack(L2Npc npc, L2PcInstance attacker, int damage,
			boolean isPet) {
		if (!npc.isDead()) {
			npc.doDie(attacker);
		}
	...
	public final String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
		...
		case VARKA_SILENOS_GREAT_MAGUS:
			dropHerb(npc, player, HP_HERBS_DROPLIST);
			if ((cond == 5) && st.hasQuestItems(ENCHSPEAR)
					&& !st.hasQuestItems(STAGE2)) {
				st.set("cond", "6");
		...
		case LATANA:
			...
			addSpawn(KETRA_ORC_SUPPORTER2, npc.getX(), npc.getY(), npc.getZ(),
					npc.getHeading(), false, 0, false, npc.getInstanceId());
		case ANTYLOPE_1:
		case ANTYLOPE_2:
		case ANTYLOPE_3:
		case ANTYLOPE_4:
	...
	private static final void teleportPlayer(L2PcInstance player, int[] coords,
			int instanceId) {
	...
	public String onExitZone(L2Character character, L2ZoneType zone) {
		if ((character instanceof L2PcInstance) && !character.isDead()
				&& !character.isTeleporting()
				&& ((L2PcInstance) character).isOnline()) {
			InstanceWorld world = InstanceManager.getInstance().getWorld(
					character.getInstanceId());
			if ((world != null) && (world.getTemplateId() == INSTANCE_ID)) {
				ThreadPoolManager.getInstance().scheduleGeneral(
						new Teleport(character, world.getInstanceId()), 1000);
			}
		}

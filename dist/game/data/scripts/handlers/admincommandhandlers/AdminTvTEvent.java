...
public class AdminTvTEvent implements IAdminCommandHandler {
	private static final String[] ADMIN_COMMANDS = { "admin_tvt_add",
			"admin_tvt_remove", "admin_tvt_advance",
			"admin_tvt_set_team1_location", "admin_tvt_set_team2_location", };
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar) {
		if (command.equals("admin_tvt_add")) {
		...
		} else if (command.equals("admin_tvt_set_team1_location")) {
			if (TvTEvent.isStarted()) {
				activeChar.sendMessage("TvT event has started!");
			}

			int[] coords = new int[3];
			coords[0] = activeChar.getX();
			coords[1] = activeChar.getY();
			coords[2] = activeChar.getZ();
			TvTEvent.setTeam1Coordinates(coords);
			activeChar.sendMessage("TvT event Team 1 location set: x:"
					+ coords[0] + " y:" + coords[1] + " z:" + coords[2]);
		} else if (command.equals("admin_tvt_set_team2_location")) {
			if (TvTEvent.isStarted()) {
				activeChar.sendMessage("TvT event has started!");
			}
			...
			coords[1] = activeChar.getY();
			coords[2] = activeChar.getZ();
			TvTEvent.setTeam2Coordinates(coords);
			activeChar.sendMessage("TvT event Team 2 location set: x:"
					+ coords[0] + " y:" + coords[1] + " z:" + coords[2]);
		}
	...
	private void add(L2PcInstance activeChar, L2PcInstance playerInstance) {
		...
		if (!TvTEvent.addParticipant(playerInstance)) {
			activeChar
					.sendMessage("Player instance could not be added, it seems to be null!");
			return;
		}

		if (TvTEvent.isStarted()) {
			new TvTEventTeleporter(playerInstance,
					TvTEvent.getParticipantTeamCoordinates(playerInstance
							.getObjectId()), true, false);
		}
	}
	...
	private void remove(L2PcInstance activeChar, L2PcInstance playerInstance) {
		...
		new TvTEventTeleporter(playerInstance,
				Config.TVT_EVENT_PARTICIPATION_NPC_COORDINATES, true, true);
	}
}
